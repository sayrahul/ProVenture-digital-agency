document.addEventListener('DOMContentLoaded', function() {
    ...
    if (submitButton) {
        ...
        submitButton.addEventListener('click', e => {
            ...
            // Gather Data
            let requestFormData = new FormData(form);

            // Construct JSON object for Vercel API fallback
            let interests = [];
            ...
            let jsonData = {
                ...
                website_url_hp: (hpField ? hpField.value : '')
            };
            ...
            const showErrorState = (msg) => {
                statusMessage.innerText = msg || "Error! Something went wrong. Please try again.";
            ...
            fetch(scriptURL, { method: 'POST', body: requestFormData })
                .then(response => {
                    showSuccessCard();
                })
                .catch(error => {
                    console.warn('Google Script submit failed, trying /api/submit endpoint...', error);
                    // Failover to Vercel API
                    fetch('/api/submit', {
                        method: 'POST',
                    ...
                    .then(apiData => {
                        if (apiData && apiData.ok !== false) {
                            ...
                            showErrorState(apiData ? apiData.error : null);
                        }
                    })
                    .catch(apiErr => {
                        console.error('Both endpoints failed:', apiErr);
                        showErrorState("Error submitting form. Please contact us directly via WhatsApp or email.");
                    });
                });
        });
    }
