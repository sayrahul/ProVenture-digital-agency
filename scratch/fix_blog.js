...
// 1. Update blog-grid CSS to exactly 4 columns
html = html.replace('grid-template-columns: repeat(auto-fill, minmax(260px, 1fr));', 'grid-template-columns: repeat(4, 1fr);');
...
// fix the string escapes:
html = html.replace(/onclick="openBlogModal\\(\\\\'blog-modal-1\\\\'\\)"/g, 'onclick="openBlogModal(\\'blog-modal-1\\')"');
html = html.replace(/onclick="openBlogModal\\(\\\\'blog-modal-2\\\\'\\)"/g, 'onclick="openBlogModal(\\'blog-modal-2\\')"');
html = html.replace(/onclick="openBlogModal\\(\\\\'blog-modal-3\\\\'\\)"/g, 'onclick="openBlogModal(\\'blog-modal-3\\')"');

// 4. Insert Blog Post 4 inside the .blog-grid container
const post4 = `
<!-- Blog Post 4 -->
...
// 5. Generate Modals HTML and place OUTSIDE <main>
const modalsHtml = `
<!-- Blog Modals -->
